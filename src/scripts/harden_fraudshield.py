...
def main():
    ...
    parser.add_argument("--v3", action="store_true", help="Train stacked FraudShield v3 (default: v2 LightGBM)")
    ...
    print("=" * 72)
    print(f"  Mode: {'v3 stacked' if args.v3 else 'v2 LightGBM'}")
    ...
    if not args.evaluate_only:
        ...
        if args.v3:
            ...
            print(f"  Total rows:    {report['mix_stats']['total_rows']}")
            print(f"  Buffer fraud:  {report['mix_stats']['buffer_fraud_rows']}")
            print(f"  Hard negatives:{report['mix_stats']['hard_negative_rows']}")
    ...
    if args.v3 and not args.evaluate_only:

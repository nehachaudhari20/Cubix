"""
Full product loop — one command for the whole Red↔Blue line.

  KB → Threat Hunter Red → Multi-surface evasion/composites →
  Evidence Buffer → Harden v3 → Evaluate → Labs → Verify

Usage:
  python scripts/run_full_loop.py
  python scripts/run_full_loop.py --families 12 --skip-train-v1
  python scripts/run_full_loop.py --skip-multi-surface
  python scripts/run_full_loop.py --multi-surface-families 10 --no-composites
  python scripts/run_full_loop.py --no-swap
...
def main():
    parser = argparse.ArgumentParser(description="Run full product Red↔Blue loop")
    parser.add_argument("--families", type=int, default=8, help="Threat Hunter hypotheses / campaigns")
    ...
    parser.add_argument(
        "--skip-multi-surface",
        action="store_true",
        help="Skip evasion search + cross-surface composites",
    )
    parser.add_argument(
        "--multi-surface-families",
        type=int,
        default=0,
        help="Limit non-payment families for evasion search (0 = all)",
    )
    parser.add_argument(
        "--no-composites",
        action="store_true",
        help="Skip multi-surface CompositeRunner chains",
    )
    ...
    print("=" * 72)
    print("FULL PRODUCT LOOP")
    print("  KB -> Red -> Multi-surface -> Buffer -> Harden v3 -> Evaluate -> Verify")
    print("=" * 72)

    sep("STEP 1 — KNOWLEDGE BASE")
    runner = LoopRunner()
    kb = runner._step_kb()
    ...
    result = runner.run(
        LoopRunConfig(
            ...
            multi_surface=not args.skip_multi_surface,
            multi_surface_families=args.multi_surface_families,
            multi_surface_composites=not args.no_composites,
            trigger="cli",
        )
    )
    ...
    print(f"  Surfaces:    {result.buffer_stats.get('surfaces', {})}")
    ms = result.multi_surface or {}
    if ms:
        print(
            f"  Multi-surf:  probes={ms.get('total_probes', 0)} "
            f"evaded={ms.get('total_evaded', 0)} "
            f"ASR={ms.get('overall_asr', 0):.1%}"
        )
    ...
    print(
        f"  Verify:      {result.verify.get('decision')} "
        f"(ml={result.verify.get('ml_score')}, v={result.verify.get('model_version')})"
    )
    print("\n  Dashboard: uvicorn backend.api.main:app --reload")
